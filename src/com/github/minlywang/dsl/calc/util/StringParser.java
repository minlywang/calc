 ...
 * <p>
 * This package methods read calc string literals and handle escaping through
 * the 'backslash' (ie: \) character. Escaping is used to neutralize string
 * delimiters (the double quotes) and read Unicode hexadecimal encoded
 * characters.
 * </p>
 * <p>
 * The only escapable character is the double quotes - '"' -, a Unicode sequence
 * starting with 'u' followed by 4 hexadecimals and the backslash character -
 * '\' - itself.
 * </p>
 * <p>
...
public class StringParser {
	...
	public static String buildString(CharSequence str, boolean eatsep) {
		...
		readAndUnescape(strb, str, begin, end, sep);
		return strb.toString();
	}

	/**
	 * Read the remainder of a string till a given separator, handles escaping
	 * through '\' syntax.
	 * 
	 ...
	 * @return the last character offset handled in origin
	 */
	private static int readAndUnescape(StringBuilder strb, CharSequence str, int begin, int end, char sep) {
		boolean escape = false;
		int index = begin;
		for (; index < end; ++index) {
			char c = str.charAt(index);
			if (escape) {
				if (c == 'u' && (index + UCHAR_LEN) < end && readUnicodeChar(strb, str, index + 1) > 0) {
					...
					// if c is not an escapable character, re-emmit the
					// backslash before it
					boolean notSeparator = sep == 0 ? c != '"' : c != sep;
					boolean unescaped = false;
					if (notSeparator && c != '\\') {
						unescaped = unescape(c, strb);
						if (!unescaped) {
							strb.append('\\');
						}
					}
	 ...
	 * Unescapes a char.
	 * 
	 * @param c the char to unescape
	 * @return is the c unescaped
	 */
	public static boolean unescape(char c, StringBuilder strb) {
